...
#define PATH "C:\\Users\\Kinaz Rezek\\Test\\ar_new\\23\\"
...
int main(int argc, char* argv[])
{
	...
	for (int ix = 1; ix <= 66; ix++)
	{
		...
		sprintf(dist, "%s_%d.sql", "isaiah", ix);
		...
		while ((c = getc(pSrc)) != EOF)
		{
			if (c == L'\n' || c == L'\r')
			{
				...
				fprintf(pDist, "INSERT INTO isaiah (chapter, verse, prefix, suffix, body)\nVALUES (%d, %d, NULL, NULL, \"%s\");\n", ix, line, strchr(buffer, ';') + 1);
