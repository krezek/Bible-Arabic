...
#define PATH "C:\\Users\\Kinaz Rezek\\Test\\ar_new\\26\\"
...
int main(int argc, char* argv[])
{
	...
	for (int ix = 1; ix <= 48; ix++)
	{
		...
		sprintf(dist, "%s_%d.sql", "ezekiel", ix);
		...
		while ((c = getc(pSrc)) != EOF)
		{
			if (c == L'\n' || c == L'\r')
			{
				...
				fprintf(pDist, "INSERT INTO ezekiel (chapter, verse, prefix, suffix, body)\nVALUES (%d, %d, NULL, NULL, \"%s\");\n", ix, line, strchr(buffer, ';') + 1);
